...
train=process_file("train.source","train.target")
test=process_file("test.source","test.target")
val=process_file("val.source","val.target")

train.to_json('data2/train.json', orient='records')
test.to_json('data2/test.json', orient='records')
val.to_json('data2/val.json', orient='records')
...
data = load_dataset("data2", data_files=data_files)
...
#        txt_file.write(tokenizer.decode(line) + "\n")


index = tokenize_data['test']['id']
