...
class SentimentExampleBatchIterator:
    ...
    def get_next_batch(self):
        if self._cur_idx < len(self.data): # loop over the dataset
            ...
            batch_inputs = batch_lengths = batch_labels = None

            # TODO: implement batch_inputs, batch_lengths, and batch_labels
            batch_inputs_1= [self.data[_idx].word_indices for _idx in range(st_idx, ed_idx)]
            end=len(batch_inputs_1)
            batch_lengths = [ len(batch_inputs_1[_idx]) for _idx in range(0, end)]
            max_length = max(batch_lengths)
            batch_inputs=[]
            for _idx in range(0, end):
                 length=max_length - batch_lengths[_idx]
                 seq=F.pad(torch.Tensor(batch_inputs_1[_idx]), pad=(0, length), mode='constant', value=0)
                 batch_inputs.append(seq.tolist())

            batch_labels = [ self.data[_idx].label for _idx in range(st_idx, ed_idx)]
